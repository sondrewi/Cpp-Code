...
/*This class will hold the indices of non-zero matrix entries in a large sparse matrix.
It consists of 2 main vectors col_index and row_index. A vector "entries" in the SparseMat
class will hold the actual coefficients of the non-zero entries of the matrix. Vector col_index
will hold the column index of each such entry in the original matrix (hence col_index size
will equal size of the entries vector). Vector row_index will contain the index (in terms of position
in the entries vector) of the first entry in each row of the original matrix. The last entry in row_index
will contain the length of the entries vector (needed when looping over entries in last row of original matrix)
Hence, the size of row_index will be nr or rows + 1*/
SparseAddress::SparseAddress(Mesh& the_mesh) : mesh(the_mesh) {
    ...
    all nbrs counted for a cell, set number of neighbours + 1 as corresponding entry
    in row_index (need to account for cell on diagonal as well).*/
    ...
    for (int i = 0; i < nr_equations; i++){
        ...
        index will equal (start index of row i + nbr non-zero entries in row i)*/
        ...
        for (int k = 0; k < nbrs.size(); k++){
            ...
            if (abs(i - nbrs[k]) > matrix_band){matrix_band = abs(i-nbrs[k]);}
        }
...
int SparseAddress::get_nr_eqs() const{
    return nr_equations;
}
...
//Return matrix band
int SparseAddress::mat_band() const{
    return matrix_band;
}
...
std::pair<int,int> SparseAddress::oneD_to_twoD(int i) const{
    ...
    //Start with guess that index i corresponds to an entry in 0th column of
    //original matrix
    int row = 0;
...
int SparseAddress::twoD_to_oneD(int i, int j, bool trunc) const{
    ...
    if (!trunc){
        ...
        //If j does not correspond to any index in relevant range stored in col_index, throw exception
        if (!found_index) {
            std::cerr << "Provided indices correspond to zero-entry in matrix" << std::endl;
            exit(EXIT_FAILURE);
        }
    }
...
int SparseAddress::get_col_index(int index) const{
    return col_index[index];
}

//getter function for entry in row_index
int SparseAddress::get_row_index(int index) const{
